        ...
        messages=[
            {"role": "system", "content": """
             ...
             {"role": "system", "content": """
              ...
              """},
              {"role": "system", "content": """
               You must answer the question in Laotian!!!!
               """},
            {"role": "user", "content": input_text},
            ]
        )
...
def convert_indonesia():
    ...
    completion = openai.ChatCompletion.create(
        ...
        messages=[
            ...
            {"role": "system", "content": "You must answer the questions in Indonesian!"},
            {"role": "user", "content": input_text},
            ]
        )
    output_text = completion.choices[0].message.content
...
@app.route('/output_dic')
def output_dic():
    output_file = os.path.join(app.root_path, 'static', 'output_dic.txt')
    with open(output_file, 'r', encoding='utf-8') as f:
        output_text = f.read()
    return render_template('output_dic.html', output_text=output_text)

@app.route('/output_suggestion')
def output_suggestion():
    output_file = os.path.join(app.root_path, 'static', 'output_suggestion.txt')
    with open(output_file, 'r', encoding='utf-8') as f:
        output_text = f.read()
    return render_template('output_suggestion.html', output_text=output_text)
...
def chinese_to_language_health():
    ...
    return redirect(url_for('output_dic'))
...
def description_health():
    ...
    with open(os.path.join(app.root_path, 'static', 'output_suggestion.txt'), 'w', encoding='utf-8') as f:
        f.write(completion.choices[0].message.content)
    return redirect(url_for('output_suggestion'))
